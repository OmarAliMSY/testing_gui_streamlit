...
def get_12bdtl(_bytearray, byte_index):
    ...
    + str(struct.unpack('B', data[2:3])[0]) + "-" \
    + str(struct.unpack('B', data[3:4])[0]) + " "\
    + str(struct.unpack('B', data[5:6])[0]) + ":" \
    + str(struct.unpack('B', data[6:7])[0]) + ":" \
    + str(struct.unpack('B', data[7:8])[0]) + "." \
...
def set_usint(_bytearray, byte_index, _int):
    """
    Set value in bytearray to int
    """
    # make sure were dealing with an int
    ...
    _bytearray[byte_index:byte_index + 1] = _bytes


def get_usint(_bytearray, byte_index):
    """
    Get int value from bytearray.
    ...
    value = struct.unpack('B', struct.pack('1B', *data))[0]
    return value
...
class DB_Row(object):
    ...
    def get_value(self, byte_index, _type):
        ...
        if _type == 'DTL':
            return get_12bdtl(_bytearray, byte_index)
