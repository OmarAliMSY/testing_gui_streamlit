...
csv_paths = glob("csv_data/231017/*.csv")
m_list = []
...
for i, csv_file in enumerate(csv_paths):
    ...
    ax.plot(df["times"], df["northSensorAnglet2"], label=f"Angle")
    ax.plot(df["times"], df["torquet2"]*110, label=f"Torque")
    #ax.plot(df["times"],df["torque_abs"]*5/2 ,label=f"Sensor Torque")
