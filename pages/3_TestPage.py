...
if check_password():
    ...
    def add_section(title, label, subsections):
            ...
            # Add a dropdown to select whether to add an image or subsection
            add_option = st.selectbox(f"Add to {title}:", ("Select", "Image", "Subsection"))
            ...
            if add_option == "Image":
                ...
                for i, subsection in enumerate(subsections[title]):
                    ...
                    for j, image in enumerate(subsection["images"]):
                        ...
                        subsection["images"][j]["caption"] = sub_caption
                        mydoc.fig(i=f"{title}_{i}_{j}", caption=sub_caption)

            # Remove Subsection button
            choice_indices = st.multiselect(options=list(range(len(subsections[title]))), label=f"Select subsections to remove from {title}", key=f"remove_subsection_{title}")
    ...
    subsections = {section: [] for section in sections}
    
    for section in sections:
        add_section(section, f"Label for {section}", subsections)